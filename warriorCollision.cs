        ...
        foreach (Collider2D enemy in hitEnemies)
        {
            ...
            if (!enemySprite.GetBool("appliedKnockback"))
            {
                ...
                else if (animator.GetBool("isDownGrounding"))
                {
                    ...
                    Vector2 knockbackForce = enemyKnockbackHandler.CalculateKnockback("DownGround", transform.position, enemy.transform.position, enemySprite.GetInteger("damage"), .6f);
                    enemyRb.AddForce(knockbackForce, ForceMode2D.Impulse);
                }
